...
class PetalPhysics {
    ...
    constructor(element, initialX, initialY, zIndex) {
        ...
        // --- NUEVAS PROPIEDADES PARA LA LLUVIA FINAL ---
        this.isFinalRain = false;
        this.targetX = 0;
        this.targetY = 0;
    ...
    updatePhysics() {
        ...
        // --- LÓGICA DE ATERRIZAJE PARA CORAZÓN (LLUVIA FINAL) ---
        if (this.isFinalRain && (this.initialY + this.y >= this.targetY)) {
            this.onGround = true;
            this.y = this.targetY - this.initialY;
            this.x = this.targetX - this.initialX;
            this.vx = 0;
            this.vz = 0;
            this.rotationX = 90; 
            this.rotationY = 0;
            this.rotation = Math.random() * 360; // Rotación aleatoria en el suelo
        } 
        else if (!this.onGround) {
            // Simular el viento lateral
            ...
            // Comprobar cilindro (solo si no es lluvia final o si aún no llega al target)
            ...
            // --- COMPROBAR SUELO NORMAL (Para los pétalos iniciales) ---
            if (!this.isFinalRain && (this.initialY + this.y > CYLINDER.bottom)) {
                this.onGround = true;
                this.y = CYLINDER.bottom - this.initialY;
                this.groundTime = 0;
                this.vy = -this.vy * 0.1; // Rebote
                this.vx *= 0.4;
                this.vz *= 0.4;
            }
        } else {
            // Comportamiento en el suelo (Asentamiento)
