...
const CANTIDAD_LLUVIA = 50; // Cuántos pétalos caerán al final
const CORAZON_PUNTOS = []; // Aquí guardaremos las coordenadas del corazón

// Generar puntos en forma de corazón para el suelo
for (let t = 0; t < Math.PI * 2; t += 0.15) {
    // Ecuación paramétrica del corazón
    const x = 16 * Math.pow(Math.sin(t), 3);
    const y = -(13 * Math.cos(t) - 5 * Math.cos(2 * t) - 2 * Math.cos(3 * t) - Math.cos(4 * t));
    CORAZON_PUNTOS.push({ x: x * 8, y: y * 8 }); // El 8 es el tamaño del corazón
}